...
REGION=eu-west-1
AWS_ACCOUNT_ID=595420848170
PROFILE=default
#PROFILE=personalAccount
DOCKER_APP_NAME=alexander_fraltsov-cart-api

DOCKER_APP_REPOSITORY=alexander_fraltsov-cart-api
...
# Tag Docker image
docker tag \
      $DOCKER_APP_REPOSITORY:"$DOCKER_TIME_TAG" \
      $AWS_DOCKER_REGISTRY_URL/$DOCKER_APP_REPOSITORY:"$DOCKER_TIME_TAG"
docker tag \
      $DOCKER_APP_REPOSITORY:$DOCKER_LATEST_TAG \
      $AWS_DOCKER_REGISTRY_URL/$DOCKER_APP_REPOSITORY:$DOCKER_LATEST_TAG

# Push/Publish Docker image
docker push \
      $AWS_DOCKER_REGISTRY_URL/$DOCKER_APP_REPOSITORY
